...
void add_value_to_register(uint32_t value, uint32_t selected_register){
	...
	reg_la0_data = (selected_register << 4| 2 & 0xF);
	reg_la1_data = value;
}

void read_value_from_register(uint32_t selected_register){

	reg_la0_data = (selected_register << 4| 1 & 0xF);
...
void main()
{
	...
	clock();
	add_value_to_triplet_register(2,4);
	clock();
	read_value_from_triplet_register(4);
	clock();
